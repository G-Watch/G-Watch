...
CUresult cuLaunchCooperativeKernel (
    ...
    if(unlikely(cuLaunchKernel_at_first_level && capsule->do_need_trace_kernel(func_name))){
        GW_IF_FAILED(
            ...
            gw_retval,
            GW_WARN(
                "failed to trace function: api(cuLaunchCooperativeKernel), err(%s)",
    ...
    if(capsule->is_capturing()){
        ...
        GW_CHECK_POINTER(cpu_event = new GWEvent("cuLaunchCooperativeKernel"));
        ...
        GW_IF_FAILED(
            ...
            {
                GW_WARN(
                    "failed to push cpu event to capsule: api(cuLaunchCooperativeKernel), err(%s)",
                    gw_retval_str(gw_retval)
                );
                delete cpu_event;
                cpu_event = nullptr;
        ...
        gpu_event->set_metadata("grid_dim_x", gridDimX);
        gpu_event->set_metadata("grid_dim_y", gridDimY);
        gpu_event->set_metadata("grid_dim_z", gridDimZ);
        gpu_event->set_metadata("block_dim_x", blockDimX);
        gpu_event->set_metadata("block_dim_y", blockDimY);
        gpu_event->set_metadata("block_dim_z", blockDimZ);
        gpu_event->set_metadata("shared_mem_bytes", sharedMemBytes);
        ...
        GW_IF_FAILED(
            ...
            {
                GW_WARN(
                    "failed to push gpu event to capsule: api(cuLaunchCooperativeKernel), err(%s)",
                    gw_retval_str(gw_retval)
                );
                delete gpu_event;
                gpu_event = nullptr;
    ...
    cudv_retval = real_cuLaunchCooperativeKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams);
    GW_DEBUG("called cuLaunchCooperativeKernel");
