...
CUresult cuLaunchKernelEx_ptsz (
    ...
    if(capsule->is_capturing()){
        ...
        GW_CHECK_POINTER(cpu_event = new GWEvent("cuLaunchKernelEx_ptsz"));
        ...
        GW_IF_FAILED(
            ...
            {
                GW_WARN(
                    "failed to push cpu event to capsule: api(cuLaunchKernelEx_ptsz), err(%s)",
                    gw_retval_str(gw_retval)
                );
                delete cpu_event;
                cpu_event = nullptr;
        ...
        GW_IF_FAILED(
            ...
            {
                GW_WARN(
                    "failed to push gpu event to capsule: api(cuLaunchKernelEx_ptsz), err(%s)",
                    gw_retval_str(gw_retval)
                );
                delete gpu_event;
                gpu_event = nullptr;
    ...
    if(gpu_event != nullptr){
        /*!
         *  \note(zhuobin): CUDA could have sticky error, and this call could fail even it returns success
         */
